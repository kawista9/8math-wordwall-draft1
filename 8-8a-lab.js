(() => {
  ...
  function finalForm(task,data,n,firstSide) {
    ...
    const part=(side,kind)=> {
      ...
      return `<label class="a88-final-part a88-${kind}"><span>${kind==="constant"?"Constant":"Variable term"}</span><span class="a88-final-entry"><select data-select="${key}-sign" aria-label="${n===1?"First way":"Reversed way"} ${esc(sideName(side))} ${kind} sign"><option value="">±</option><option value="+" ${data[key+"-sign"]==="+"?"selected":""}>+</option><option value="-" ${data[key+"-sign"]==="-"?"selected":""}>−</option></select><input type="number" min="0" step="any" inputmode="decimal" data-input="${key}" value="${esc(data.inputs[key]||"")}" aria-label="${n===1?"First way":"Reversed way"} ${esc(sideName(side))} ${kind} amount">${kind==="variable"?"<b>· x</b>":""}</span></label>`;
    };
    const side=code=>`<div class="a88-final-side"><strong>${esc(sideName(code))}</strong>${part(code,"constant")}${part(code,"variable")}</div>`;
    return `<div class="a88-form"><h6>${n===1?"First way":"Same comparison, sides reversed"}</h6><div class="a88-form-line">${side(firstSide)}${select("f"+n+"-symbol",data["f"+n+"-symbol"]||"",[["=","="],["<","<"],[">",">"],["≤","≤"],["≥","≥"]],`${n===1?"First way":"Reversed way"} symbol`)}${side(secondSide)}</div></div>`;
  }
